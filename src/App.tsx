...
const App: React.FC = () => {
  ...
  if (isInGame) {
    return (
      <div className="sketch-scene h-screen text-zinc-200 overflow-hidden font-sans">
        <div className="relative z-10 h-full grid grid-cols-[1fr_340px] max-[860px]:grid-cols-[1fr]">
          <main className="relative flex flex-col min-w-0">
            {/* Header */}
            <header className="h-14 md:h-16 px-3 md:px-5 border-b border-zinc-800/80 bg-black/55 backdrop-blur flex items-center justify-between">
              <div className="flex items-center gap-2 md:gap-3">
                <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-zinc-100 text-black flex items-center justify-center font-black text-sm">{Math.max(1, game.roundCount)}</div>
                <div>
              ...
              <div className="flex items-center gap-1 md:gap-2">
                ...
                <input
                  ...
                  className="w-10 md:w-16 h-9 cursor-pointer accent-zinc-200"
                ...
                />
                <button onClick={() => game.setIsMuted(!game.isMuted)} className="icon-button" title={displayLanguage === 'zh' ? '静音/取消静音' : 'Mute / Unmute'} aria-label={displayLanguage === 'zh' ? '静音/取消静音' : 'Mute / Unmute'}>{game.isMuted ? <VolumeX className="w-3.5 h-3.5 md:w-4 md:h-4" /> : <Volume2 className="w-3.5 h-3.5 md:w-4 md:h-4" />}</button>
                <button onClick={() => { game.setPhase(GamePhase.LOBBY); rec.setShowRecords(true); }} className="icon-button" title={displayLanguage === 'zh' ? '返回大厅' : 'Return to lobby'} aria-label={displayLanguage === 'zh' ? '返回大厅' : 'Return to lobby'}><RefreshCw className="w-3.5 h-3.5 md:w-4 md:h-4" /></button>
              </div>
            </header>
          ...
          {/* Log sidebar — hidden on narrow screens, shown on wide */}
          <aside className="bg-zinc-950/88 border-l border-zinc-800 flex flex-col min-h-0 max-[860px]:hidden">
            <div className="h-14 md:h-16 px-4 border-b border-zinc-800 flex items-center justify-between">
                ...
                {game.logs.map(log => (
                  <div key={log.id} className={`log-entry-in ${log.isSystem ? 'text-center' : log.speakerId === MY_PLAYER_ID ? 'text-right' : 'text-left'}`}>
                    {log.isSystem ? (
                      ...
                      <div className={`inline-block max-w-[220px] md:max-w-[270px] rounded-lg border p-1.5 md:p-2 text-xs leading-relaxed ${log.speakerId === MY_PLAYER_ID ? 'bg-zinc-100 text-black border-zinc-200' : 'bg-zinc-900 border-zinc-700 text-zinc-200'}`}>
          ...
          {/* Mobile log drawer (narrow screens) — overlay when toggled */}
          <div className="hidden max-[860px]:block fixed bottom-4 right-4 z-30">
            <button
              onClick={() => rec.setShowRecords(!rec.showRecords)}
              className="icon-button bg-zinc-900 border-zinc-600 shadow-lg"
              title={displayLanguage === 'zh' ? '对局日志' : 'Game log'}
              aria-label={displayLanguage === 'zh' ? '对局日志' : 'Game log'}
            >
